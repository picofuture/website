...
A few days ago, <a href="https://x.com/karpathy" target="_blank">Andrej Karpathy</a> released a <a href="https://www.youtube.com/watch?v=7xTGNNLPyMI" target="_blank">video</a> titled "Deep dive into LLMs like ChatGPT." It’s packed with useful stuff, but it’s also 3 hours and 31 minutes long. 
...
- **You're trying to fine-tune your own LLM** but keep running into confusing stuff like `chat_template` and `ChatML` (especially if you're using <a href="https://axolotl-ai-cloud.github.io/axolotl/docs/dataset-formats/conversation.html" target="_blank">Axolotl</a>).
...
- There’s an optimal number of symbols (tokens) for compression. For example, GPT-4 uses **100,277** tokens.
...
In order to fully open-source a base model, two things are required:
  - **<a href="https://github.com/openai/gpt-2/blob/master/src/model.py">The code</a>** (e.g., a few hundred lines in Python that defines the steps the model will take to generate the prediction).
...
One such dataset is <a href="https://huggingface.co/datasets/OpenAssistant/oasst1" target="_blank">OASST1</a>. Early post-training datasets were **hand-curated by humans**. 
...
The model isn't just trying to immitate the human but it's coming up with it's own strategies through trial and error to win the game.
...
of it's own to express it's thoughts and ideas because it disovered that it's the best way to express it's thoughts and ideas.

### Learning on unverifiable domains a.k.a Reinforcement Learning from Human Feedback (RLHF)
It is very easy to exclude humans from the RL process in verifiable domains. LLMs can act as a judge for it's own performance.
...
The LLM will generate the jokes without issues, but judging their quality at is not possible.
...
In order for doing RLHF at scale, you basically train a separate reward model which could be a transformer without the extra layers surrounding it. 
...
#### RLHF Downside
We are doing RL with respect to a lossy simulation of humans. We NEED a reward model that is close to the real human preferences but it's still a simulation not the real thing. It might be misleading!
...
This bad example that goes through the NN of the model and basically sabotages the whole process is called <a href="https://en.wikipedia.org/wiki/Adversarial_machine_learning" target="_blank">Adversarial Examples</a>.

You cannot really take these examples and give them a low score in the training data because there are infinite number of examples that the system can produce.

The ideal of training the reward model is to train it for a few 100 iterations then crop it there. If you train it further, it will start to game the system and the performance will start to decrease.

RLHF can be considered RL in the systems with deterministic outputs. Like the game of Go. But if there is a way to game the system, like in LLMs, it's not exactly RL but rather a way to further fine-tune the model.
...
- <a href="https://lmarena.ai" target="_blank">LM Arena</a>.
- <a href="https://buttondown.com/ainews" target="_blank">AI News</a>.
- <a href="https://x.com" target="_blank">X</a> (Twitter).

## Where to find them
- Proprietary: OpenAI, Google, Anthropic, etc.
- Open weight models (DeepSeek, Meta, etc.): An inference provider i.e. <a href="https://together.ai" target="_blank">Together</a>.
- Run them locally: <a href="https://ollama.com/" target="_blank">Ollama</a>, <a href="https://lmstudio.ai/" target="_blank">LM Studio</a>, etc.
- Base models: <a href="https://app.hyperbolic.xyz" target="_blank">Hyperbolic</a>.
